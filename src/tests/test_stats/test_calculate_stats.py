...
import turning_point.metric_stats.calculate_metric_stats as cms
from tournament_simulations.data_structures import Matches
from turning_point.metrics.variances import Variances
...
def test_calculate_quantile__one():
    ...
    assert cms._calculate_mean_quantile(test, 0.95).equals(expected)
...
def test_get_kwargs_from_variances():
    ...
    assert cms.get_kwargs_from_metric(test, 0.95)["df"].equals(expected)
    ...
    expected = pd.DataFrame(
        {
            ...
            "real": [np.var([0, 6], ddof=1), np.var([2, 2, 1, 1], ddof=1)],
            "mean": [mean(*first_id_vars), mean(*second_id_vars)],
            "quantile": [nfp(*first_id_vars), nfp(*second_id_vars)],
        }
    ).set_index("id")

    result = cms.get_kwargs_from_matches(
        test,
...
def test_get_kwargs_from_matches_bigger_in_to_probabilities_new_points():
    ...
    expected = pd.DataFrame(
        {
            "id": pd.Categorical(["1", "2"]),
            "real": [np.var([2, 4], ddof=1), np.var([0, 0, 0, 0], ddof=1)],
            "mean": [mean(*first_id_vars), mean(*second_id_vars)],
            "quantile": [nfp(*first_id_vars), nfp(*second_id_vars)],
        }
    ).set_index("id")
